...
export class Player {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number) {
    ...
    this.sprite = scene.add.sprite(x, y, 'prologue-mc', 0).setDepth(5).setScale(0.12);
  ...
  update(): void {
    ...
    const horizontalDirection = this.resolveAxisDirection(
      [this.cursors.left, this.wasd.A],
      [this.cursors.right, this.wasd.D]
    );
    const verticalDirection = this.resolveAxisDirection(
      [this.cursors.up, this.wasd.W],
      [this.cursors.down, this.wasd.S]
    );
    ...
    this.updateSpriteAnimation(vx, vy);
  ...
  private resolveAxisDirection(
    negativeKeys: Phaser.Input.Keyboard.Key[],
    positiveKeys: Phaser.Input.Keyboard.Key[]
  ): -1 | 0 | 1 {
    const latestNegative = this.getLatestDownKeyTime(negativeKeys);
    const latestPositive = this.getLatestDownKeyTime(positiveKeys);

    if (latestNegative === null && latestPositive === null) return 0;
    if (latestNegative === null) return 1;
    if (latestPositive === null) return -1;
